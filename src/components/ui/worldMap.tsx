...
export default function WorldMap({ locale, countries }: WorldMapProps) {
  ...
  useLayoutEffect(() => {
    ...
    worldSeries.mapPolygons.template.events.on("click", (ev) => {
      ...
      const dataItem =
        polygon.dataItem as am5.DataItem<am5map.IMapPolygonSeriesDataItem> | null;
      if (!dataItem) return;
      ...
      const zoomPromise = zoomAnimation
      ...
      Promise.all([zoomPromise]).then(async () => {
        try {
          ...
          console.error("GeoJSON の読み込みに失敗:", e);
        }
      });
    });
    ...
    backContainer.states.create("hover", {
      opacity: 0.35,
      scale: 1.05,
    });
    ...
    homeButton.events.on("click", () => {
      if (currentDataItem) countrySeries.zoomToDataItem(currentDataItem);
      else chart.goHome();
    });
