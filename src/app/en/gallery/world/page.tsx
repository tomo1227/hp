...
export default async function Page() {
  ...
  const galleries = await getFilteredPosts({ locale: "en", dateOrder: "desc" });
  ...
  const labels = {
    title: "World Gallery",
    lead: "Pick a country from the map to jump into its travel stories. Richer color means more posts, and the top list gives you quick routes to popular destinations.",
    stats: {
      countries: "Countries Visited",
      galleries: "Galleries",
      top: "Top Countries",
      japanTop: "Top Japan Prefectures",
    },
    mapTitle: "World Map",
    mapHint:
      "Darker color means more posts. Click a country to open the gallery.",
    listTitle: "Country Index",
    listHint: "Select a country to view its gallery collection.",
            ...
            <div className="world-top">
              ...
              <div className="world-top-list">
                {topCountries.map((item) => (
                  <Link
                    key={item.country}
                    href={`/en/gallery/world/${item.country}`}
                    className="world-top-chip"
                  >
                    <span className="world-top-name">{item.country}</span>
